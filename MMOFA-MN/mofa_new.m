...
if isempty(varargin)
    ...
    tMax = 200; %最大迭代次数
    nEl = 200; %精英解最大容量
    %  MMF  SYM_PART_simple  SYM_PART_rotated Omni_test
    Problem = 'MMF14';
else
    time = varargin{1};
    n = varargin{2};
...
% 小生境初始化
niches = niches_init(position, cost, pn);
...
while t <= max_evaluate
    ...
    if ismerge(niches) == 0
        temp = niches_eval(niches, nEl,d,m);
        niches = niches_init(temp(:,1:d), temp(:,d+1:d+m),pn);
    end
